...
export function Reframe({ t }: { t: HomeContent["reframe"] }) {
  ...
  return (
    <section className="mx-auto max-w-[1240px] px-5 py-28 sm:py-36">
      <span aria-hidden className="gilt-rule block mb-20 opacity-30" />
      <div className="grid items-center gap-12 lg:grid-cols-[0.95fr_1.05fr]">
        <Reveal y={0} x={-28} duration={0.95}>
          <SectionHeading
            ...
            as="h2"
          />
          <p className="mt-6 max-w-[60ch] text-pretty text-[length:var(--text-lead)] leading-relaxed text-ink-soft">{t.note}</p>
        </Reveal>
        ...
        <div className="relative mx-auto aspect-[4/3] w-full max-w-[520px]">
          ...
          >
            {positions.map((p, i) => (
              <line
                ...
                x2={50}
                y2={50}
                stroke="oklch(0.62 0.215 294 / 0.22)"
                strokeWidth="0.4"
              />
            ))}
          </svg>
          ...
          <motion.div
            ...
            className="absolute left-1/2 top-1/2 z-[2] grid h-28 w-28 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-aurora text-center [box-shadow:inset_0_0_0_1px_oklch(0.83_0.13_88_/_0.55),inset_0_1px_0_oklch(1_0_0_/_0.2),0_0_70px_-4px_oklch(0.6_0.2_290_/_0.75)]"
          >
            <span className="px-2 font-[family-name:var(--font-display)] text-base font-bold leading-tight text-ink">
              {t.osLabel}
            </span>
          </motion.div>
