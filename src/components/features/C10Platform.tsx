    ...
    <Section tint className="py-20 sm:py-28">
      ...
      <div className="mt-12 grid gap-6 lg:grid-cols-[1.35fr_1fr]">
        ...
        <div className="flex flex-col gap-6">
          ...
          <Reveal y={26}>
            <div className="overflow-hidden rounded-2xl bg-bg-deep ring-line" dir="ltr">
              <div className="flex items-center gap-3 border-b border-line px-4 py-2.5">
                ...
                <span className="ms-auto flex gap-1.5">
                  <span className="h-2.5 w-2.5 rounded-full bg-[oklch(0.34_0.045_266)]" />
                  <span className="h-2.5 w-2.5 rounded-full bg-[oklch(0.34_0.045_266)]" />
                </span>
              </div>
          ...
          <Reveal y={24} delay={0.05}>
            <div className="rounded-2xl bg-surface/40 p-5 ring-line">
              <div className="text-xs font-semibold text-muted">{t.webhookLabel}</div>
              <div className="mt-3 flex flex-wrap gap-2" dir="ltr">
                {t.webhooks.map((w) => (
                  <span
                    key={w}
                    className="rounded-md bg-bg/70 px-3 py-1.5 font-mono text-xs text-ink-soft ring-line"
                  >
                    {w}
                  </span>
                ))}
              ...
              <div className="mt-5 flex flex-col gap-2.5 sm:flex-row sm:items-stretch">
                {t.nodes.map((node, i) => (
                  <div key={i} className="flex flex-1 items-stretch gap-2.5 sm:flex-col">
                    <div
                      className={`flex flex-1 flex-col rounded-xl p-3.5 ring-line ${
                        i === 0 ? "bg-[oklch(0.62_0.2_264_/_0.12)]" : "bg-bg/60"
                      }`}
                    >
                      <span
                        className={`text-[11px] font-semibold uppercase tracking-wide ${
                          i === 0 ? "text-primary-bright" : "text-gold"
                        }`}
                      >
                        {node.label}
                      </span>
        ...
        <div className="flex flex-col gap-6">
            ...
            <PhoneFrame>
              <div className="flex flex-col gap-3 p-4 pt-9">
                <div className="flex items-center gap-2.5">
                  <span className="grid h-9 w-9 place-items-center rounded-lg bg-aurora glow-gold text-xs font-semibold text-ink">
                    CF
                  </span>
                  <div>
                ...
                <div className="rounded-xl bg-surface/60 p-3 ring-line">
                  <div className="relative aspect-video overflow-hidden rounded-lg bg-[oklch(0.13_0.04_264)]">
                    <div
                      aria-hidden
                      className="absolute inset-0"
                      style={{
                        background:
                          "radial-gradient(100% 80% at 30% 0%, oklch(0.6 0.25 300 / 0.3), transparent 60%)",
                      }}
                    />
                    <span className="absolute bottom-1.5 left-1.5 rounded bg-bg/70 px-1.5 py-0.5 text-[9px] text-ink-soft ring-line">
                ...
                <div className="flex flex-col gap-1.5">
                  {t.mobileFeatures.map((f) => (
                    <div
                      key={f}
                      className="flex items-center gap-2 rounded-lg bg-surface/40 px-3 py-2 text-[11px] text-ink-soft ring-line"
                    >
          ...
          {/* Marketplace */}
          <Reveal y={24} delay={0.1}>
            <div className="rounded-2xl bg-[oklch(0.62_0.2_264_/_0.1)] p-6 [box-shadow:inset_0_0_0_1px_oklch(0.62_0.2_264_/_0.4)]">
              ...
              <div className="mt-4 flex -space-x-2 rtl:space-x-reverse">
                {["AI", "UX", "BIZ", "DEV", "+9"].map((x) => (
                  <span
                    key={x}
                    className="grid h-8 w-8 place-items-center rounded-full bg-surface text-[10px] font-semibold text-ink-soft ring-2 ring-bg"
                  >
                    {x}
                  </span>
                ))}
